...
class ControleUpdateStatus {
	async handle(request: Request, response: Response) {
		const { opt_cod_cliente, opt_chave_ped, app, novo_status } =
			request.body;
		//
		console.log(
			`Pedido At. ${opt_chave_ped} Status: ${novo_status} CodCli: ${opt_cod_cliente}`
		);
		//
		if (app === "ACCON") {
			...
			if (!ped) {
				...
				var UaiResponse = await fetch(
					`https://www.uairango.com/api2/auth/pedido/${opt_chave_ped}`,
					requestOptionsUai
				);
