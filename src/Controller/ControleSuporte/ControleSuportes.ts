...
class ControleListaSuporte {
	async handle(request: Request, response: Response) {
		...
		if (!dataInicial) {
			nextUrl =
				next < total ? `${currentURL}?offset=${next}${segueURL}` : null;
		} else {
			nextUrl =
				next < total
					? `${currentURL}?offset=${next}&dataInicial=${dataInicial}&dataFinal=${dataFinal}${segueURL}`
					: null;
		}
		//
		...
		if (!dataInicial) {
			previousURL =
				previous != null
					? `${currentURL}?offset=${previous}${segueURL}`
					: null;
		} else {
			previousURL =
				previous != null
					? `${currentURL}?offset=${previous}&dataInicial=${dataInicial}&dataFinal=${dataFinal}${segueURL}`
					: null;
		}
		//
